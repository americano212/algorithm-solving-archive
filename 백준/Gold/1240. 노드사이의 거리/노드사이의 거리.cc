#include <bits/stdc++.h>
#define FOR(x,y,n)  for(int x=y;x<(n);x++) 
#define endl '\n';
#define INF 2'147'483'647LL
#define ll long long
#define ld long double
#define pii pair<int, int>
#define pll pair<ll, ll>
#define pllp pair<ll, pll>
#define pff pair<float, float>
#define pdd pair<double, double>
#define pldd pair<ld, ld>
#define matrix vector<vector<ll>>
#define lcm(a, b) a * b / gcd(a, b)
#define all(v) v.begin(), v.end()
#define btw(x,a,b) a<=x && x<b
ll gcd(ll a, ll b) { return (b ? gcd(b, a % b) : a); }

const ll mod = 1'000'000'007LL;
int dx[4] = { -1,0,1,0 };
int dy[4] = { 0,-1,0,1 };
int dx6[6] = { 1,-1,0,0,0,0 };
int dy6[6] = { 0,0,1,-1,0,0 };
int dz6[6] = { 0,0,0,0,1,-1 };
int dx8[8] = { 0,-1,-1,-1,0,1,1,1 };
int dy8[8] = { 1,1,0,-1,-1,-1,0,1 };

using namespace std;

struct EDGE { ll start; ll end; ll cost; };
struct DOT3 { ll x; ll y; ll z; };
struct NODE { ll max_value; int max_index; ll now_value; int now_node; };
struct ITEM { bool isNum; int num = 0; char bracket = ' '; };
struct MOVE { int x; int y; int time; };

int n, m;

ll bfs(matrix& mat, int start, int end) {
    vector<bool> visited(n + 1, false);
    visited[start] = true;

    queue<pll> qu;
    qu.push({ start,0 });

    while (!qu.empty()) {
        ll now_node = qu.front().first;
        ll now_cost = qu.front().second;
        qu.pop();
        if (now_node == end) return now_cost;

        FOR(next_node, 1, n + 1) {
            if (mat[now_node][next_node] != 0) {
                ll next_cost = now_cost + mat[now_node][next_node];
                if (!visited[next_node]) {
                    visited[next_node] = true;
                    qu.push({ next_node,next_cost });
                }
            }
        }
    }
}

void solve() {
    cin >> n >> m;
    int a, b, dist;
    matrix mat(n + 1, vector<ll>(n + 1, 0));
    FOR(_, 0, n - 1) {
        cin >> a >> b >> dist;
        mat[a][b] = dist;
        mat[b][a] = dist;
    }


    FOR(_, 0, m) {
        cin >> a >> b;
        cout << bfs(mat, a, b) << endl;
    }
}

int main() {
    cin.tie(0)->sync_with_stdio(0);

    ll tc = 1;
    // cin >> tc;
    while (tc--)solve();

    return 0;
}